...
import {
  Receipt, RefreshCw, Loader2, AlertTriangle, Wifi, CheckCircle2, ChevronDown, Search,
} from 'lucide-react';
import { api, ApiTransaction, PaymentSearchResult } from '../api/client';
import { FilterChips, toast } from './ui';
...
export default function TransactionsView() {
  ...
  // Bumped to reload the DataTable (auto-refresh tick, or after a reconnect).
  ...
  const loadQueue = () => api.transactions.unconnected().then((r) => setUnconnected(r.results)).catch(() => {});

  // Refresh the stuck queue + the table every 10 seconds.
  useEffect(() => {
    loadQueue();
    const t = window.setInterval(() => { loadQueue(); reload(); }, 10_000);
  ...
  const reconnectAll = async () => {
    ...
    for (const tx of unconnected) {
      try { await api.transactions.reconnect(tx.id); ok += 1; } catch { /* keep going */ }
    }
  ...
  const columns: Column<ApiTransaction>[] = [
    {
      header: 'Time', sortKey: 'created_at',
      render: (tx) => (
    ...
    { header: 'Phone', render: (tx) => <span className="font-mono">{tx.phone}</span> },
    { header: 'Plan', render: (tx) => tx.plan_name },
    {
      header: 'Amount', sortKey: 'amount', className: 'text-right',
      render: (tx) => <span className="font-mono font-bold whitespace-nowrap">KSh {Number(tx.amount).toLocaleString()}</span>,
    },
    {
    ...
    {
      header: 'Connected',
      render: (tx) => (tx.status !== 'success' && tx.status !== 'reconciled') ? (
        ...
        >
          {reconnecting === tx.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <Wifi className="h-3 w-3" />}
          reconnect
        </button>
      ) : (
    ...
    { header: 'Receipt', render: (tx) => <span className="font-mono">{tx.mpesa_receipt || '—'}</span> },
    {
      header: 'Detail',
      render: (tx) => <span className="text-[#141414]/60 max-w-[16rem] truncate inline-block align-bottom" title={tx.result_desc}>{tx.result_desc || '—'}</span>,
    },
  ];
  ...
  return (
    <div className="space-y-6 text-[#141414]">
      ...
      {/* The stuck-customer queue: paid but never got online. */}
      {unconnected.length > 0 && (
        <div className="border border-[#B26B00] bg-[#FFF8EC]">
          ...
          >
            ...
            <ChevronDown className={`h-4 w-4 text-[#B26B00] shrink-0 transition-transform ${queueOpen ? 'rotate-180' : ''}`} />
          </button>
            ...
            <>
              <div className="flex items-center justify-between gap-3 px-3 py-2 border-t border-[#B26B00]/25 bg-[#FFF3DC]/50">
                ...
                {unconnected.length > 1 && (
                  ...
                  >
                    {reconnectingAll ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Wifi className="h-3.5 w-3.5" />}
                    Reconnect all
                  </button>
                )}
              </div>
              <div className="divide-y divide-[#B26B00]/15">
                {unconnected.map((tx) => (
                  <div key={tx.id} className="flex items-center gap-3 px-3 py-2.5 text-xs">
                    ...
                    >
                      {reconnecting === tx.id || reconnectingAll ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Wifi className="h-3.5 w-3.5" />}
                      Reconnect
                    </button>
                  </div>
                ))}
      ...
      {/* Unified search across hotspot + PPPoE payments. */}
      ...
      {searchActive ? (
        ...
        <DataTable<ApiTransaction>
          fetcher={(q) => api.transactions.list(q)}
          columns={columns}
          rowKey={(tx) => tx.id}
          searchable={false}
